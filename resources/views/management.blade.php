    ...
    <div class="table-container">
        ...
        <div class="table-placement">
            <table id="table" class="table">
                ...
                @foreach($autoparkList as $item)
                    <tr class="table-row" data-type="Самосвал (20-25т)">
                        <td><input type="checkbox" id="{{$item->garageNumber}}" class="checkbox" onclick="Check()"></td>
                        <td onclick="window.location.href = '/vehicle-card/'+{{$item->garageNumber}}"><img src="/img/icons/vehicle_photo.png" alt="vehicle-icon"></td>
                        <td onclick="window.location.href = '/vehicle-card/'+{{$item->garageNumber}}">{{$item->vehicleName}}</td>
                        <td onclick="window.location.href = '/vehicle-card/'+{{$item->garageNumber}}">{{$item->garageNumber}}</td>
                        <td onclick="window.location.href = '/vehicle-card/'+{{$item->garageNumber}}">{{$item->vahicleType}}</td>
                        <td class="vehicle-object" onclick="window.location.href = '/vehicle-card/'+{{$item->garageNumber}}">{{$item->vehicleObject}}</td>
                        <td onclick="window.location.href = '/vehicle-card/'+{{$item->garageNumber}}">
                            @if($item->status == 'В рейсе')
                                <div class="vehicle-status in-trip">
                                    <span>В рейсе</span>
                                </div>
                            @elseif($item->status == 'В перебазировке')
                                <div class="vehicle-status in-relocation">
                                    <span>В перебазировке</span>
                                </div>
                            @elseif($item->status == 'В техн. простое')
                                <div class="vehicle-status in-idle">
                                    <span>В техн. простое</span>
                                </div>
                            @elseif($item->status == 'В простое б. в.')
                                <div class="vehicle-status in-idle">
                                    <span>В простое б.в.</span>
                                </div>
                            @elseif($item->status == 'В ремонте')
                                <div class="vehicle-status on-repair">
                                    <span>В ремонте</span>
                                </div>
                            @elseif($item->status == 'На хранении (И)')
                                <div class="vehicle-status in-storage">
                                    <span>На хранении (И)</span>
                                </div>
                            @elseif($item->status == 'На хранении (НИ)')
                                <div class="vehicle-status in-storage">
        ...
        <div class="table-footer">
            ...
            <span>Всего техники: {{count($autoparkList)}}</span>
        ...
        <div class="pop-up-container">
            <div class="pop-up-body">
                ...
                <form action="/axiosInsert" method="post">
                    @csrf
                    <input type="text" id="сompany" name="company" value="{{$company}}" hidden>
